...
public class SettingSlider : MonoBehaviour
{
    ...
    public void Load()
    {
        ...
        {
            int intValue = Convert.ToInt32(PlayerSettings.instance.GetSetting(key, type));
            float floatValue = (float)intValue * 0.01f;
            GetComponent<Slider>().value = floatValue;
        } else if (type == SettingType.Float)
        {
            GetComponent<Slider>().value = Convert.ToSingle(PlayerSettings.instance.GetSetting(key, type));
        }
    }
    ...
    public void SetSetting(float value)
    {
        ...
        {
            PlayerSettings.instance.SetSetting(key, type, value.ToString());
        }
        else if (type == SettingType.Int)
        {
            int intValue = (int)(value * 100.0f);
            PlayerSettings.instance.SetSetting(key, type, intValue.ToString());
        }
    }
}
